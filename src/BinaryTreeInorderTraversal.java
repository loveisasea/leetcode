...
public class BinaryTreeInorderTraversal {
	...
	public static void main(String[] args) {
		...
		TreeNode input = ins.new TreeNode(4);
		input.left = ins.new TreeNode(1);
		input.left.right = ins.new TreeNode(3);
		input.left.right.left = ins.new TreeNode(2);
		input.right = ins.new TreeNode(6); 
		input.right.left = ins.new TreeNode(5); 
		String result = "[1,2,3,4,5,6]";
		Util.printInput(input); 
		Util.printResult(ins.listToString(ins.inorderTraversal(input)), result);
	}
